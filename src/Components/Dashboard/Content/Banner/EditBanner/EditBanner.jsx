...
import { addBanner, editBanner } from '../../../../../features/dashboard/action';
...
function EditBanner({edit , setEdit}) {
    ...
    <div className='flex flex-col w-[93%] sm:w-full opacity-motion 2xl:w-[70%]'>
        <div className='w-full bg-[#C0D9DB] p-2 flex justify-between'>
            ...
            <MdCancel className='text-red-600 font-bold text-3xl transition-all hover:text-red-500' onClick={(e)=>setEdit({})}/>
        </div>
        <form className='flex flex-col items-center bg-[#ffffff70] px-2 py-5 w-full gap-8 z-10 opacity-90' onKeyDown={(e)=>formKeyNotSubmit(e)}>
           {/* title */}
            <div className='flex flex-col gap-2 w-full'>
                ...
                <input type="text" className='p-1  outline-[#0ab694] w-full' ref={titleRef} required={true} name='title' defaultValue={edit.title}/>
            </div>
               {/* image */}
            <div className='flex flex-col gap-2 w-full'>
