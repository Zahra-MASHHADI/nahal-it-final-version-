...
import { getWorkSample, getWorkSampleCategories } from '../../features/dashboard/action';
        ...
        >
            ...
            {workSampleCategories.map(work => (
                ...
                {  work.children.length > 0 ?
                 ...
                 onMouseEnter={()=>{clickEnterShowSlide11(work.id)}}
                 onMouseLeave={clickOutShowSlide11}
                 key={work.id}
             >
                 <Link to={`workSamples/${work.id}/${work.title}`}  className="text-[#171717] font-[shabnamMedium] text-sm cursor-pointer"> {work.title} </Link>
                 <MdKeyboardArrowLeft />
                 <div
                 onMouseEnter={()=>{clickEnterShowSlide11(work.id)}}
                 className={
                    hoveredItem === work.id
                    ? "absolute bg-white -left-[12rem] -top-2 px-7 py-2 shadow-[1px_1px_10px_rgba(0,0,0,.25)] flex flex-col gap-y-2 items-center justify-center transition-all duration-700"
                    : "absolute bg-white -left-[12rem] -top-2 px-7 py-2 shadow-[1px_1px_10px_rgba(0,0,0,.25)] flex flex-col gap-y-2 items-center justify-center opacity-0 -translate-y-[50rem] transition-all duration-700"
                 }
                 >
                    {work.children.map(item=>(
                        
                            <Link to={`workSamples/${item.id}/${item.title}`}  className="px-4 py-1" key={item.id} >
                    ...
                    <li>
                        <Link to={`/workSamples/${work.id}/${work.title}` } className="flex items-center justify-start py-2" key={work.id}>
                            <p className="text-[#171717] font-[shabnamMedium] text-sm cursor-pointer">  {work.title}</p>
                        </Link>
                    </li>
                    :
                    <></>
